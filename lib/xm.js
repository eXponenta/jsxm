...
export class XMPlayer extends XMEffects {
    ...
    async load(arrayBuf) {
        ...
        if (this.messager) {
            await this.messager.notify('load', { buffer: arrayBuf }, [arrayBuf]);
            this.buffer = null;
        }
        return true;
    }
    ...
    async init() {
        ...
        if (this.audioctx.createScriptProcessor === undefined) {
            ...
            this.messager = new AsyncMessager(this.worker.port);
        }
        ...
        if (this.buffer) {
            const status = await this.messager.notify('load', { buffer: this.buffer }, [this.buffer]);
